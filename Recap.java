...
/**
 * cette classe permet d'afficher le recapitulatif des cours pour un etudiant
 */
public class Recap extends JPanel
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if(droit==0)
        {
            n=getttseance(ide);//recupérer les cours auxquelle l'etudiant est inscrit
        }
        ...
        for(int i=0;i<n.size();i++)//lecteur de la liste des seance
        {
            ...
            cours=z+"   -   "+getprof(z);//obtenir le nom du proffesseur
            g2.drawString(cours, 20, 71+i*50);
            id=getpremierjour(z,ide,droit,"");//obtnire la date et l'heure de la premiere seance
            ids=getids(id);
            jour=getjour(id);
            jour=datestring(jour,ids,id);//transformer toutes les infos en string
            g2.drawString(jour, 554, 71+i*50);//afficher les infos
            id=getdernierjour(z,ide,droit,"");//obtnire la date et l'heure de la derniere seance
            ids=getids(id);
            jour=getjour(id);
            jour=datestring(jour,ids,id);//transformer toutes les infos en string
            g2.drawString(jour, 1087, 71+i*50);//afficher les infos
            nb=getnbseance(ide,z,droit,"");//obtenir le nombre de seance
            heure=nbtotime(nb);//obtenir le nombre d'heure de cours 
            g2.drawString(nb+"", 1750, 71+i*50);//afficher les infos 
